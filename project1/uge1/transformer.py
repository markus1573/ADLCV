...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        attention_logits = torch.bmm(queries, keys.transpose(1, 2))  # (b*h) x seq_len x seq_len
...
class PositionalEncoding(nn.Module):
    def __init__(self, embed_dim, max_seq_len=512):
        ...
        pe[:, 0::2] = torch.sin(position * div_term)  # assign to even positions
        pe[:, 1::2] = torch.cos(position * div_term)  # assign to odd positions
