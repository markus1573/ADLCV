...
class Attention(nn.Module):
    ...
    def forward(self, x, return_attention=False):
        ...
        if return_attention:
            # Reshape attention: (batch_size * num_heads) x seq_len x seq_len -> batch_size x num_heads x seq_len x seq_len
            attention = rearrange(attention, '(b h) s1 s2 -> b h s1 s2', h=self.num_heads)
...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x, return_attention=False):
        if return_attention:
            attention_out, attn_weights = self.attention(x, return_attention=True)
        else:
...
class ViT(nn.Module):
    ...
    def get_attention_maps(self, img):
        ...
        Returns a list of attention maps, one per layer.
        Each attention map has shape: batch_size x num_heads x seq_len x seq_len
        """
        tokens = self.to_patch_embedding(img)
        batch_size, num_patches, embed_dim = tokens.size()
